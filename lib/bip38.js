...
Bip38.prototype.decryptRaw = function(encData, passphrase) {
  ...
  assert.equal(encData.length, 39, "Invalid private key")

  // first byte is always 0x01
  assert.equal(encData[0], 0x01, 'Invalid private key')
  encData = encData.slice(1)
  ...
  if (!isECMult) {
    ...
    var passPoint = new Buffer(curve.G.multiply(new BigInteger(passFactor.toString('hex'), 16)).getEncoded(true))
