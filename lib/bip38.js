...
Bip38.prototype.decrypt = function(encryptedKey, passphrase) {
  ...
  if (!isECMult) {
    ...
    if (hasLotSeq) {
      ...
      var passFactor = sha256.x2(hashTarget, {in: 'buffer', out: 'buffer'});
    } else {
      var passFactor = preFactor;
    }
    var passPoint = new Buffer(ecparams.getG().multiply(new BigInteger(passFactor.toString('hex'), 16)).getEncoded(true));
    ...
    var factorB = sha256.x2(seedB, {in: 'buffer', out: 'buffer'});
    
    var privKey = new BigInteger(passFactor.toString('hex'), 16).multiply(new BigInteger(factorB.toString('hex'), 16)).mod(ecparams.getN());
